...
typedef struct {
    ...
    // press" and "keep it subtle" are separate wishes.
    uint8_t vtx_sent_style;
} setting_elrs_t;

typedef enum {
    SETTING_VTX_SENT_OSD_ALWAYS = 0,
    SETTING_VTX_SENT_OSD_DELIBERATE = 1,
    SETTING_VTX_SENT_OSD_OFF = 2,
} setting_vtx_sent_osd_t;

typedef enum {
    SETTING_VTX_SENT_STYLE_NORMAL = 0,
    SETTING_VTX_SENT_STYLE_SUBTLE = 1,
