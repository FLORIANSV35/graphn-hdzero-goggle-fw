...
// Show/hide the FPGA-drawn recording square (reg 0x8d bit0). Unlike the LVGL
// OSD this is a primitive rendered inside the FPGA, so it needs no OSD plane
// lock and works on HDMI In at every frame rate.
void osd_show_hdmi_in_dvr(uint8_t is_show) {
    uint8_t reg;

    reg = I2C_Read(ADDR_FPGA, 0x8d);
    // Keyed off is_show, not dvr_is_recording: the caller decides. Reading the
    // global here made osd_show_hdmi_in_dvr(0) a no-op whenever recording was
    // still active, so the "clear it on the way out" call could leave the
    // square lit.
    if (is_show)
        reg |= 0x01;
    else
